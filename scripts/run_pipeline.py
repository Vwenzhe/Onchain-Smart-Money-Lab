...
def main() -> None:
    ...
    run_step("Step 1/5: 渲染 SQL", render_sql_main)
    run_step("Step 2/5: 拉取 Dune 结果", run_dune_queries_main)
    run_step("Step 3/5: 生成特征层", build_feature_layer_main)
    run_step("Step 4/5: 生成地址画像", build_address_profiles_main)
    run_step("Step 5/5: 生成 Token AI 总结", build_token_ai_summary_main)
    print("\nPipeline 执行完成。")
